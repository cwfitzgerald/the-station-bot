...
lazy val meta = project.in(file("."))
    .aggregate(launcher, `the-station-bot`)
...
lazy val launcher = project.in(file("launcher"))
    .settings(Seq(
	    ...
	    libraryDependencies ++= Seq(
		    "io.methvin" %% "directory-watcher-better-files" % "0.9.3"
	    ),
	    scalacOptions ++= scalaOptions,
	    scalacOptions in (Compile, console) --= Seq("-Ywarn-unused:imports"),
	    assemblyJarName in assembly := "launcher.jar",
	    mainClass in assembly := Some("com.cwfitz.the_station_bot.launcher.Main")
    ))

lazy val `the-station-bot` = project.in(file("the-station-bot"))
    .settings(Seq(
	    version := "0.1",
	    scalaVersion := "2.12.8",
	    libraryDependencies ++= Seq(
